...
// TestCrossAccountCreateOrphanedENIOnDeleteFailure verifies that when both
// CreateNetworkInterfacePermission and the subsequent cleanup DeleteNetworkInterface
// fail, the original permission error is still returned to the caller and the
// delete attempt is recorded — the ENI is acknowledged as orphaned via a log warning.
func TestCrossAccountCreateOrphanedENIOnDeleteFailure(t *testing.T) {
	permErr := errors.New("simulated permission grant failure")
	delErr := errors.New("simulated delete failure")

	remote := ec2mock.NewAPI(remoteSubnets, remoteVpcs, nil, noRouteTables)
	tracking := &callTrackingAPI{EC2API: remote, permissionErr: permErr, deleteErr: delErr}
	local := ec2mock.NewAPI(localSubnets, localVpcs, nil, noRouteTables)
	client := newTestCrossAccountClient(t, local, tracking, "111122223333")

	eniID, eni, err := client.CreateNetworkInterface(t.Context(), 5, "subnet-remote", "test-eni", nil, false)

	assert.ErrorIs(t, err, permErr, "original permission error should be propagated even when delete also fails")
	assert.Empty(t, eniID)
	assert.Nil(t, eni)

	require.Len(t, tracking.permissionCalls, 1)
	require.Len(t, tracking.deleteCalls, 1, "delete should have been attempted despite the permission error")
	assert.Equal(t, tracking.permissionCalls[0].eniID, tracking.deleteCalls[0])
}
